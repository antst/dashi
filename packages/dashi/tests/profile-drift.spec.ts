...
describe('validated DSH patch surface', () => {
  it('keeps dashi overrides and preset composition aligned with installed DSH', () => {
    ...
    // Authorization supports /login, Schedule owns /loop, and Sessionbus bridges peers; none belongs to the web client.
    for (const row of rowsAt(dashi, 4).filter(row => !['authorization', 'schedule', 'dashi', 'sessionbus', 'roller'].includes(row.id))) {
