...
const probe = [
  'printf "ARG:%s\\n" "$@"',
  'printf "GROUPS:%s\\n" "${SESSIONBUS_GROUPS-UNSET}"',
  'printf "TOKEN:%s\\n" "${SESSIONBUS_LAUNCH_TOKEN-UNSET}"',
  'printf "OTHER:%s\\n" "${DASHI_OTHER-UNSET}"',
].join('\n')
...
describe('dashi launcher', () => {
  ...
  it('uses the sessionbus profile when the launch token is set', () => {
    const directory = fakePath(probe)
    const result = spawnSync(process.execPath, [launcher, '--resume'], {
      encoding: 'utf8', env: launcherEnv(directory, { SESSIONBUS_LAUNCH_TOKEN: 'secret-token' }),
    })
  ...
  it('collects repeatable group forms and forwards only the remaining arguments', () => {
    ...
    const result = spawnSync(process.execPath, [
      launcher, '--model', 'm', '-g', 'one, two', '--group', 'three', '-g=four, five',
      '--group=six, seven', '--verbose',
    ], { encoding: 'utf8', env: launcherEnv(directory) })
    expect(result.stdout).toContain('ARG:--profile\nARG:dashi\nARG:--model\nARG:m\nARG:--verbose\n')
    expect(result.stdout).toContain('GROUPS:["one","two","three","four","five","six","seven"]\n')
    expect(result.status).toBe(0)
  })
  ...
  it.each([
    ...
    expect(result.status).toBe(2)
    expect(result.stdout).toBe('')
    expect(result.stderr).toBe('dashi: -g requires a value\n')
  })
