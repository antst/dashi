...
| `Ctrl+C` twice | idle, empty composer | exit |
| `Ctrl+D` twice | idle, empty composer | exit; shares the Ctrl+C arm |
...
/model  /permission  /agents  /queue  /context  /tasks
...
job/subagent details; `/export` writes controller history as Markdown.
